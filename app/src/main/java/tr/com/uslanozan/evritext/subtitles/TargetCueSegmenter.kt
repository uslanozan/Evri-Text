...
object TargetCueSegmenter {
    ...
    fun segment(sentence: Sentence, translation: String): List<Cue> {
        ...
        val anchors = sentence.sourceCues
            ...
            .distinct()
            .sorted()
        val boundaries = chooseBoundaries(sentence, pieces, anchors)
        ...
        return cues.takeIf(::isReadable) ?: listOf(sentence.asCue(clean))
    }
    ...
    internal fun targetPieces(text: String): List<String> {
        ...
        return sentences.flatMap(::breakLongPiece).filter(String::isNotEmpty)
    }
        ...
        for (pieceIndex in 0 until pieces.lastIndex) {
            ...
            val candidates = anchors.filter { anchor ->
                anchor > previous &&
                    ...
                    anchor !in selected &&
                    isWithinReadingSpeed(pieces[pieceIndex], anchor - previous) &&
    ...
    private fun isReadable(cues: List<Cue>): Boolean = cues.all { cue ->
        cue.durationMs >= MIN_EVENT_MS &&
            ...
            isWithinReadingSpeed(cue.text, cue.durationMs)
    }

    private fun isWithinReadingSpeed(text: String, durationMs: Long): Boolean =
        durationMs > 0 && text.length * 1000.0 / durationMs <= MAX_CHARS_PER_SECOND
